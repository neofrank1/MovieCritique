...
export const searchTVShows = async (req, res) => {
    try {
        ...
        const options = {
            method: 'GET',
            url: `https://api.themoviedb.org/3/search/tv?query=${encodeURIComponent(query)}&language=en-US&page=${page || 1}`,
            headers: {
                accept: 'application/json',
                Authorization: `Bearer ${process.env.TMDB_ACCESS_TOKEN}`
            }
        ...
        if (response.status !== 200) {
            console.error('Error searching TV shows:', response.statusText);
            return res.status(response.status).json({ error: 'Failed to search TV shows' });
        }
