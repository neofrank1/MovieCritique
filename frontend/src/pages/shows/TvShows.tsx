...
export default function MovieShows() {
    ...
    useEffect(() => {
        setIsLoading(true);
        let endpoint = `http://localhost:3000/api/movies/popular-tv?page=${page}`;

        if (isSearching && searchQuery.trim() !== "") {
            endpoint = `http://localhost:3000/api/movies/searchTV?query=${searchQuery}&page=${page}`;
        }
    ...
    return (
        <>
            <Header />
            <AppPageLayout>
                <h1 className="text-4xl font-bold text-center mt-10 mb-5">TV Shows</h1>
                <p className="text-center text-lg mb-10">Get a sneak peek at the most anticipated TV shows on the horizon. Explore the popular releases, and mark your calendar for the next big hits in television!</p>
                        ...
                        TVShows.results.map((tv: any) => (
                            ...
                            >
                                ...
                                <p className="text-center">
                                    {overviewTruncate(tv.overview, 200) ? overviewTruncate(tv.overview, 200) : "No overview available."}
                                </p>
                                <p className="font-extrabold text-md justify-center ">
                                    {dateFormatter(tv.first_air_date)}
                                </p>
                            </Card>
                        ))
                    )}
